...
mod tests {
    ...
    fn test_keyboard_state() {
        // Key-name resolution is disabled in this vendored copy (it called
        // macOS TIS APIs that crash off the main thread). The state tracker
        // must therefore return no names, but must not panic.
        let mut keyboard = Keyboard::new().unwrap();
        assert_eq!(keyboard.add(&EventType::KeyPress(Key::KeyS)), None);
        assert_eq!(keyboard.add(&EventType::KeyRelease(Key::KeyS)), None);
